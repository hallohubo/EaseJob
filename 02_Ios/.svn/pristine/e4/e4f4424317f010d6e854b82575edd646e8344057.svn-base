...
@interface TZImagePickerController () <UIGestureRecognizerDelegate>{
    NSTimer *_timer;
    UILabel *_tipLabel;
...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = [UIColor colorWithRed:(234/255.0) green:(138/255.0) blue:(88/255.0) alpha:1.0];
    self.oKButtonTitleColorDisabled = [UIColor colorWithRed:(234/255.0) green:(138/255.0) blue:(88/255.0) alpha:0.5];
    
    self.navigationBar.barTintColor = [UIColor whiteColor];
    self.navigationBar.tintColor = [UIColor colorWithRed:(51/255.0) green:(51/255.0) blue:(51/255.0) alpha:1.0];
...
- (void)configDefaultSetting {
    ...
    self.naviTitleColor = [UIColor colorWithRed:31 / 255.0 green:40 / 255.0 blue:51 / 255.0 alpha:1.0];
    if (@available(iOS 8.2, *)) {
        self.naviTitleFont = [UIFont systemFontOfSize:17.0 weight:UIFontWeightSemibold];
    } else {
        // Fallback on earlier versions
        self.naviTitleFont = [UIFont systemFontOfSize:17.0];
    }
    if (@available(iOS 8.2, *)) {
        self.barItemTextFont = [UIFont systemFontOfSize:17.0 weight:UIFontWeightMedium];
    } else {
        // Fallback on earlier versions
        self.barItemTextFont = [UIFont systemFontOfSize:17.0];
    }
    self.barItemTextColor = [UIColor colorWithRed:51 / 255.0 green:51 / 255.0 blue:88 / 255.0 alpha:1.0];
    ...
    self.iconThemeColor = [UIColor colorWithRed:234 / 255.0 green:138 / 255.0 blue:88 / 255.0 alpha:1.0];
...
//    viewController.automaticallyAdjustsScrollViewInsets = NO;
    ...
    {
        //为下一个界面统一设置返回按钮样式
        viewController.navigationItem.leftBarButtonItem = [self itemWithNormalImage:[UIImage imageNamed:@"icon_back_black"] highLightImage:[UIImage imageNamed:@"icon_back_black"] target:self action:@selector(back)];
...
- (UIBarButtonItem *)itemWithNormalImage:(UIImage *)normalImage highLightImage:(UIImage *)highLightImage target:(id)target action:(SEL)action
{
    UIButton * btn = [UIButton buttonWithType:UIButtonTypeCustom];
    [btn setImage:normalImage forState:UIControlStateNormal];
    [btn setImage:highLightImage forState:UIControlStateHighlighted];
    btn.contentEdgeInsets = UIEdgeInsetsMake(0, -26, 0, 0);
    //绑定事件
